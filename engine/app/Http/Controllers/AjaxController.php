...
class AjaxController extends Controller
{
	...
	{
		if ($request->has("q") && $request->q != "" && $request->q != "null" && $request->q != "" && $request->q != "null") {
	...
	{
		if ($request->has("q") && $request->q != "" && $request->q != "null") {
			if($request->q != ""){
	...
	{
		if ($request->has("q") && $request->q != "" && $request->q != "null") {
			// $response = $this->getData("inventory-property/" . $request->q . "/search");
			// return response()->json($response);
			$response = $this->client()->get("inventory-property/" . $request->q . "/search");
			$hasil = json_decode($response->getBody())->data;
			return response()->json($hasil);
		} else {
			// $response = $this->getData("inventory-property");
			// return response()->json($response);
