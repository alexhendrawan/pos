		...
		<form action="{{url('/')}}/penerimaan" method="post">
			...
			<div class="row">
				...
				<div class="col-md-6">
					...
					<div class="form-group">
						Dibayar
						<h4>Rp {{ number_format($data->paid_total) }}</h4>
					</div>
				</div>
			</div>
			<div class="col-md-12 box">
				<div class="col-md-12" id="bbb" >
					...
					<table id="tabel" class="table table-bordered table-stripped">
						<thead>
							<th>No.</th>
							<th>Barang</th>
							<th>Qty</th>
						...
						<tbody id="myTable">
							...
							<tr>
								...
								<td>{{$key->qty}} {{$key->poline->satuan->name}}</td>
								<td>{{number_format($key->price_per_satuan_id) ?? 0}}</td>
								<td>{{number_format($key->sell_per_satuan_id)?? 0}}</td>
								<td>{{number_format($key->price_per_satuan_id * $key->qty) ?? 0}}</td>
