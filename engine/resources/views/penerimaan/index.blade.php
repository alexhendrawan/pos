			...
			<table id="tabel" class="table table2excel table-bordered table-hover box">
				...
				<tbody id="item-table">
					...
					<tr >
						...
						<td onclick="detail({{$key -> id}},'penerimaan')">{{$key->poheader->supplier->supplier_name ?? ""}}</td>
						...
						<td onclick="detail({{$key -> id}},'penerimaan')">{{date("d-m-Y",strtotime($key->invoice_date))}}</td>
						...
						<td class=""><button class="btn btn-xs btn-danger dis"
												onclick="konfirmasi({{$key->id}},'penerimaan')"
